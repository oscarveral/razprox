...
def trimf(x: np.ndarray, a: float, b: float, c: float) -> np.ndarray:
    ...
    """
    assert(a <= b <= c), "Invalid parameters for triangular membership function. Must satisfy a <= b <= c."
    assert(isinstance(x, np.ndarray)), "Input must be a numpy array."
    assert(x.ndim == 1), "Input array must be one-dimensional."
    assert(np.issubdtype(x.dtype, np.number)), "Input array must contain numeric values."
...
def trapmf(x: np.ndarray, a: float, b: float, c: float, d: float) -> np.ndarray:
    ...
    Returns:
        np.ndarray: Membership values.
    """
    assert(a <= b <= c <= d), "Invalid parameters for trapezoidal membership function. Must satisfy a <= b <= c <= d."
    assert(isinstance(x, np.ndarray)), "Input must be a numpy array."
    assert(x.ndim == 1), "Input array must be one-dimensional."
    assert(np.issubdtype(x.dtype, np.number)), "Input array must contain numeric values."
    return np.clip(np.minimum(np.minimum((x - a) / (b - a), 1), (d - x) / (d - c)), 0, 1)
