...
class FuzzySet:
    ...
    def __init__(
        self, name: str, membership_function: callable, type: str = "real"
    ):
        ...
        self._mf_applier = (
            self._mf_real if type == "real" else self._mf_discrete
        )
    ...
    def _mf_real(self, values: np.ndarray | float) -> np.ndarray:
        ...
        assert isinstance(
            values, np.ndarray
        ), "Input must be a numpy array or a float"
...
if __name__ == "__main__":
    ...
    fuzzy_set = FuzzySet(
        "Test Set",
        lambda x: np.clip(1 - np.abs(x - 10) / 10, 0, 1),
        type="real",
    )
